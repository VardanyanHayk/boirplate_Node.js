...
class RefreshToken {
  ...
  async refresh (token) {
    try {
      ...
      const uuid = uuidv4();
      const authToken = jwt.sign(JSON.stringify({ id: decoded.id, password: decoded.password, exp: Math.floor(Date.now() / 1000) + 60 }), secret)
      const refreshToken = jwt.sign(JSON.stringify({ id: decoded.id, password: decoded.password, type: 'refresh', exp: Math.floor(Date.now() / 1000) + 60 * 60 * 48, uuid }), secret)
      await RefreshTokens
        .query()
        .update({
          uuid,
