...
class User {
  async create(req, res, next) {
    try {
      ...
      const user = await Users.query().insertAndFetch(data);
  ...
  async update(req, res, next) {
    try {
      ...
      if (data.oldPassword && data.newPassword) {
        ...
        return response(res, 200, 'Password successfully changed', {
          authToken,
  ...
  async updateForgotPassword(req, res, next) {
    try {
      ...
      const refresh = await refreshToken.create(jwtData);
      response(res, 200, 'Password successfully changed', {
        authToken,
        refreshToken: refresh,
      });
    } catch (err) {
      ...
      if ((err.name = 'TokenExpiredError'))
        return response(res, 400, 'Time out');
      next(err);
    }
  }
  ...
  async uploadImage(req, res, next) {
    try {
      ...
      const [update] = await Users.query()
        .patch({
